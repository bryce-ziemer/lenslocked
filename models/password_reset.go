...
// validate token and make sure not expired make sure have user information for that token
// once validated, delete the password reset
func (service *PasswordResetService) Consume(token string) (*User, error) {
	// Query for the password reset in our db
	tokenHash := service.hash(token)
	var user User
	...
	if err != nil {
		...
		return nil, fmt.Errorf("consume - error scanning: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("consume - error deleting: %w", err)
	}
